...
- [ ] T001 Record the current green baseline: run `pytest -q` and note the passing count, so
  Phase 6 can prove no regression (SC-006).

## Phase 2: Foundational (blocking prerequisites)

- [ ] T002 In `tests/test_cli.py`, add a reusable helper that scaffolds a temp git repo and drives
...
- [ ] T003 [US1] In `src/specguard/cli.py`, replace the inline `roles_text` built in
...
- [ ] T004 [US1] Ensure the generated roles.yml still round-trips: `parse_roles` runs on the active
  (uncommented) content before write, and the added comments do not break parsing (FR-007).
- [ ] T005 [P] [US1] In `tests/test_cli.py`, add tests asserting the generated `.specguard/roles.yml`
...
- [ ] T006 [US2] In `src/specguard/cli.py`, rewrite `CONFIG_TEMPLATE` so every key (`watch`,
...
- [ ] T007 [P] [US2] In `tests/test_cli.py`, add tests asserting the generated `.specguard/config.yml`
...
- [ ] T008 [US3] In `src/specguard/cli.py`, import `REGIONS_PATH` from `specguard.config` and add a
...
- [ ] T009 [US3] In `_offer_optional_files`, add a `regions.yml` offer mirroring the config/roles
...
- [ ] T010 [US3] In `_cmd_init`, drop `locked_at`/`locked_by` from the generated `lock.json` dict
...
- [ ] T011 [P] [US3] In `tests/test_cli.py`, add tests: (a) `regions.yml` is offered and, when
...
- [ ] T012 [P] Update the README **Configuration** section so it reflects that `init` now offers
  `regions.yml` and that generated `lock.json` omits the metadata fields — keep it consistent with
  the actual generated templates.
- [ ] T013 [P] Fix pre-existing doc drift found during planning: `SPECGUARD_PRODUCT_SPEC.md` §9
  Phase 0 lists an `additive_changes` rule key the parser does not implement — correct it to
  `edit` / `scope_changes` only.
- [ ] T014 Full gate: `ruff check .` + `mypy` clean; `pytest -q` green at the T001 baseline + the
